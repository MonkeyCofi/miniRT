...
/*   Updated: 2024/12/05 18:40:20 by pipolint         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline t_bool	is_in_shadow(t_minirt *minirt, t_tuple point)
{
	...
	s.new_point = subtract_tuples(&point, &minirt->light->position);
...
t_tuple	shade(t_minirt *minirt, t_inter_comp *intersect_comp)
{
	...
	final_res = (t_tuple){0};
	shadow = is_in_shadow(minirt, intersect_comp->point_adjusted);
	res = lighting(intersect_comp, minirt->light, shadow, minirt);
