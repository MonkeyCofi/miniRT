...
/*   Updated: 2024/10/20 17:39:40 by ahaarij          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_minirt	*init_default(t_mlx *mlx)
{
	...
	// 	return (NULL);
	// }
	// minirt->lights = ft_calloc(2, sizeof(t_light *));
	// if (!minirt->lights)
	// {
	// 	free(minirt->spheres);
	// 	free(minirt);
	...
	/* LIGHT */
						...
						// if (!minirt->lights[0])
						// 	return (NULL);
						// minirt->lights[0]->intensity = return_tuplepoint(1, 1, 1, COLOR);
						// minirt->lights[0]->position = return_tuplepoint(-10, 10, -10, POINT);
						
						// minirt->lights[1] = ft_calloc(1, sizeof(t_light));
						// if (!minirt->lights[1])
						// 	return (NULL);
						// minirt->lights[1]->intensity = return_tuplepoint(0.2, 0.2, 0.2, COLOR);
						// minirt->lights[1]->position = return_tuplepoint(0, -5, -5, POINT);
	/* LIGHT */
	...
	// 	return (NULL);
	// t_tuple	c;
	// c = return_tuple(0.8, 1, 0.6, COLOR);
	...
	//minirt->plane->transform = identity();
									...
									*sphere->material->color = return_tuple(0.1, 0.2, 0.6, COLOR);
	...
	// *cylinder->material->color = return_tuple(0.7, 0, 0, COLOR);
	// minirt->shapes[1] = ft_calloc(1, sizeof(t_shape));
	...
	// minirt->shapes[1]->type = CYLINDER;
						...
						*sphere_two->material->color = return_tuple(0.5, 0.1, 0, COLOR);
